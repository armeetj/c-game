...
state_t *emscripten_init() {
  ...
  printf("%s\n", "To toggle dev-mode (view all acceleration vectors), hold the D key");
  ...
  for(size_t i = 0; i < NUM_DIAMONDS; i++) {
    vector_t position = vector(rand_range(MIN_POSITION.x, WINDOW.x), rand_range(MIN_POSITION.y, WINDOW.y));
    ...
    list_t *d_properties = body_get_properties(diamond);
    list_add(d_properties, color);
...
void emscripten_main(state_t *state) {
  ...
  if (scene_get_dev_mode(state->scene)) {
    ...
    for (size_t i = 0; i < scene_bodies(scene); i++) {
      body_t *body = scene_get_body(scene, i);
      body_set_color(body, *((rgb_color_t*) list_get(body_get_properties(body), 0)));
    }
  }

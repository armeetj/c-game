...
vec_list_t *make_circle(size_t radius, size_t num_points, size_t center_x,
                        size_t center_y) {
  double curr_angle = 0;
  double vert_angle = 2 * PI / num_points;
  double x;
  double y;
  vec_list_t *vertices = vec_list_init(num_points);
  ...
  for (size_t i = 0; i < num_points; i++) {
    ...
    vec_list_add(vertices, temp_vector);
...
state_t *emscripten_init() {
  ...
  state->star = make_star(CIRCLE_RAD, CENTER.x, CENTER.y, NUM_STAR_POINTS);
  state->ds.x = 1;
  state->ds.y = 1;
  state->d_theta = 0.01;
  return state;
